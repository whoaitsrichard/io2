...
est_2.x
a_3 = (a-est_2.x[0])/est_2.x[1]

# Plot values and estimated histogram
truncated_bids_2  = [x for x in bid_df.submitted_bids if x> a]
truncated_values_2  = [x for x in bid_df.value if x> a]
plt.hist(truncated_bids_2,bins=50,density=True,color='skyblue',edgecolor='black', alpha=0.4,label="Bids")
plt.hist(truncated_values_2,bins=50,density=True,color='forestgreen',edgecolor='black', alpha=0.4,label="Values")
grid = np.linspace(bid_df.value.min(), bid_df.value.max(), 400)
plt.plot(grid, truncnorm.pdf(grid,a=a_3,b=b, loc=est_2.x[0], scale=est_2.x[1]), linewidth=2)
plt.xlabel('Valuations')
plt.ylabel('Frequency')
plt.title('Truncated Values and Est. Distribution, 1st & 3rd Order Method ')
plt.legend()
plt.show()
